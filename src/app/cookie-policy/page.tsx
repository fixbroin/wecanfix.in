...
import type { GlobalWebSettings, FirestoreSEOSettings } from "@/types/firestore"; // Removed ContentPage as it's not specific to this
...
import Breadcrumbs from '@/components/shared/Breadcrumbs'; // For breadcrumbs
import type { BreadcrumbItem } from '@/types/ui'; // For breadcrumbs type

export const dynamic = 'force-dynamic';

const PAGE_SLUG = "cookie-policy"; // Used for canonical URL and fallback title
...
export async function generateMetadata(
  ...
  const ogImage = webSettings?.websiteIconUrl || webSettings?.logoUrl || seoSettings.structuredDataImage || `${appBaseUrl}/default-og-image.png`;
  ...
  return {
    ...
    openGraph: {
      ...
      url: canonicalUrl,
      siteName: siteName,
      type: 'website', // Or 'article' if you prefer
      images: ogImage ? [{ url: ogImage }] : [],
    },
  };
}

// Helper to fetch all global settings for the page component if needed beyond policy content
...
export default async function CookiePolicyPage() {
  const policyData = await getCookiePolicyData();
  const pageTitleForDisplay = "Cookie Policy"; // Consistent title
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="max-w-3xl mx-auto">
        <div className="mb-8">
          ...
          {/* No "last updated" for cookie policy unless specifically tracked */}
        </div>
