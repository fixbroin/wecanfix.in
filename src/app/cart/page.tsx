...
function CartPageContent() {
  ...
  const handleQuantityChange = (itemId: string, newQuantity: number) => {
    ...
    if (newQuantity === 0) {
      ...
      logUserActivity(
        'removeFromCart',
        { serviceId: itemBeingChanged.id, serviceName: itemBeingChanged.name, quantity: itemBeingChanged.quantity },
        user?.uid,
                ...
                <div className="flex-grow">
                  ...
                  <div className="flex items-baseline gap-2 mt-1">
                    {(item.discountedPrice !== undefined && typeof item.discountedPrice === 'number' && item.discountedPrice < item.price) ? (
                      <>
                        <p className="text-sm sm:text-md font-medium text-primary">₹{item.discountedPrice.toLocaleString()}</p>
