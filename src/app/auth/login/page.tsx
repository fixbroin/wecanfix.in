...
export default function LoginPage() {
  ...
  const onGoogleSubmit = async () => {
    try {
      ...
      // The auth/popup-closed-by-user error is caught inside signInWithGoogle
      // So no need to handle it here again. Any other error will be toasted from there.
    ...
    const setupRecaptcha = () => {
      if (typeof window !== 'undefined' && !recaptchaVerifierRef.current) {
        ...
        if (recaptchaContainer) {
            recaptchaVerifierRef.current = new RecaptchaVerifier(auth, recaptchaContainer, { 'size': 'invisible' });
            recaptchaVerifierRef.current.render().catch((error) => {
              console.warn("reCAPTCHA render failed initially, will try again on submit.", error);
            });
        }
      }
    };
