...
import { Megaphone, Save, Loader2, Settings, BarChart2, FacebookIcon, Code, Send, MessageCircle, Database, Smartphone, KeyRound, Server } from "lucide-react";
...
const defaultMarketingValues: MarketingSettingsFormData = {
  ...
  whatsAppApiToken: "", whatsAppPhoneNumberId: "", whatsAppBusinessAccountId: "", whatsAppVerifyToken: "",
};
            ...
            <TabsContent value="whatsapp">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Enter your credentials from Meta for Business to send automated messages.</CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  <FormField control={form.control} name="whatsAppApiToken" render={({ field }) => (<FormItem><FormLabel>API Access Token</FormLabel><FormControl><Input type="password" placeholder="Your WhatsApp permanent API token" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  <FormField control={form.control} name="whatsAppPhoneNumberId" render={({ field }) => (<FormItem><FormLabel>Phone Number ID</FormLabel><FormControl><Input placeholder="ID of the phone number to send from" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  <FormField control={form.control} name="whatsAppBusinessAccountId" render={({ field }) => (<FormItem><FormLabel>WhatsApp Business Account ID</FormLabel><FormControl><Input placeholder="Your WABA ID" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  <FormField control={form.control} name="whatsAppVerifyToken" render={({ field }) => (<FormItem><FormLabel>Webhook Verify Token</FormLabel><FormControl><Input placeholder="Create a secure, random string" {...field} /></FormControl><FormDescription>This token is used to verify webhook GET requests from Meta.</FormDescription><FormMessage /></FormItem>)} />
                </CardContent>
              </Card>
            </TabsContent>
