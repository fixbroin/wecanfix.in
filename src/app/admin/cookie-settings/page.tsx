...
export default function CookieSettingsPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const scriptId = 'tinymce-cookie-script'; // Unique ID for this instance
      if (!document.getElementById(scriptId)) {
        const script = document.createElement('script');
        script.id = scriptId;
        script.src = "https://cdn.jsdelivr.net/npm/tinymce@6/tinymce.min.js"; // Use your TinyMCE source
        script.referrerPolicy = "origin";
        script.async = true;
        script.onload = () => setTinymceLoaded(true);
        script.onerror = () => toast({ title: "Editor Error", description: "Could not load rich text editor.", variant: "destructive" });
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleSaveSettings)}>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      {tinymceLoaded ? (
                        <Editor
                          apiKey={process.env.NEXT_PUBLIC_TINYMCE_API_KEY || 'YOUR_TINYMCE_API_KEY'} // Replace with your actual API key
                          value={field.value}
                          ...
                          init={{
                            ...
                            toolbar: 'undo redo | blocks | bold italic forecolor backcolor | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image media table | code | removeformat | help',
                            content_style: 'body { font-family:Helvetica,Arial,sans-serif; font-size:14px }',
                            images_upload_handler: (blobInfo, progress) => new Promise((resolve, reject) => {
                                toast({
                                    title: "Image Upload Not Supported Here",
                                    description: "Please host your images externally and link them using the 'Insert/edit image' by URL option.",
                                    variant: "destructive", duration: 7000,
                                });
                                reject('Image uploads are not configured for this editor instance.');
                            }),
                          }}
                          disabled={isSaving || !tinymceLoaded}
                        />
                      ) : (
          ...
          <CardFooter className="mt-6 border-t pt-6 flex justify-end">
            <Button type="submit" disabled={isSaving || !tinymceLoaded}>
