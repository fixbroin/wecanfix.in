...
import type { FirestoreBooking, BookingStatus, BookingServiceItem, AppSettings, ProviderApplication, FirestoreNotification } from '@/types/firestore';
import { db } from '@/lib/firebase';
import { collection, query, orderBy, onSnapshot, doc, updateDoc, Timestamp, deleteDoc, where, getDocs, deleteField, addDoc, getDoc as getFirestoreDoc } from "firebase/firestore"; // Added addDoc, getFirestoreDoc
...
import { generateInvoicePdf as generateInvoicePdfForDownload } from '@/lib/invoiceGenerator'; // Renamed for clarity
...
export default function AdminBookingsPage() {
  ...
  const prepareAndSendEmail = async (
    ...
    emailType: 'booking_confirmation' | 'booking_completion' | 'booking_rescheduled',
    ...
    try { const emailResult = await sendBookingConfirmationEmail(emailInput); if (emailResult.success) { toast({ title: "Email Sent", description: `${emailType.replace('_', ' ')} email sent to customer.` }); } else { toast({ title: "Email Failed", description: emailResult.message || `Could not send ${emailType.replace('_', ' ')} email.`, variant: "destructive", duration: 7000 }); } return emailResult.success;
  ...
  const handleStatusChange = async (booking: FirestoreBooking, newStatus: BookingStatus) => {
    ...
    if (requiresPaymentMethodConfirmation && newStatus === "Completed") { setSelectedBookingForPaymentUpdate(booking); setIsPaymentMethodDialogOpen(true);
      ...
      try { const updatePayload: Record<string, any> = { status: newStatus, updatedAt: Timestamp.now() }; if (newStatus === "Completed" && booking.isReviewedByCustomer === undefined) { updatePayload.isReviewedByCustomer = false; } await updateDoc(bookingDocRef, updatePayload); toast({ title: "Success", description: `Booking status updated to ${newStatus}.` });
        if (newStatus === "Completed") { const bookingForEmail = { ...booking, status: newStatus }; await prepareAndSendEmail(bookingForEmail, 'booking_completion'); }
  ...
  const handleConfirmPaymentAndUpdateStatus = async () => {
    ...
    try { await updateDoc(bookingDocRef, updatePayload); toast({ title: "Success", description: `Booking marked as Completed. Payment via ${paymentReceivedMethodForDialog}.` });
  ...
  const handleDeleteBooking = async (bookingId: string) => {
    ...
    setIsDeleting(bookingId); const bookingDocRef = doc(db, "bookings", bookingId);
    try { await deleteDoc(bookingDocRef); toast({ title: "Success", description: `Booking ${bookingId} deleted.` });
    } catch (error) { console.error("Error deleting booking: ", error); toast({ title: "Error", description: "Could not delete booking.", variant: "destructive" });
  ...
  const generateAdminRawTimeSlots = (referenceDate: Date, config: AppSettings): string[] => {
    ...
    const servicePeriodsToUse = [
        { period: 'morning', startTime: config.timeSlotSettings?.morning?.startTime || defaultAppSettings.timeSlotSettings.morning.startTime, endTime: config.timeSlotSettings?.morning?.endTime || defaultAppSettings.timeSlotSettings.morning.endTime },
        { period: 'afternoon', startTime: config.timeSlotSettings?.afternoon?.startTime || defaultAppSettings.timeSlotSettings.afternoon.startTime, endTime: config.timeSlotSettings?.afternoon?.endTime || defaultAppSettings.timeSlotSettings.afternoon.endTime },
        { period: 'evening', startTime: config.timeSlotSettings?.evening?.startTime || defaultAppSettings.timeSlotSettings.evening.startTime, endTime: config.timeSlotSettings?.evening?.endTime || defaultAppSettings.timeSlotSettings.evening.endTime },
    ];
    servicePeriodsToUse.forEach(periodConfig => {
        if (!periodConfig.startTime || !periodConfig.endTime) return; let currentSlotTimeMinutes = parseTimeToMinutes(periodConfig.startTime); const periodEndTimeMinutes = parseTimeToMinutes(periodConfig.endTime);
        while (currentSlotTimeMinutes < periodEndTimeMinutes) { const currentSlotDateTime = new Date(referenceDate); currentSlotDateTime.setHours(Math.floor(currentSlotTimeMinutes / 60), currentSlotTimeMinutes % 60, 0, 0); if (currentSlotDateTime >= earliestBookableAbsoluteTime) { slots.push(formatTimeFromMinutes(currentSlotTimeMinutes)); } currentSlotTimeMinutes += slotInterval; }
    }); return slots;
  };
  ...
  const handleConfirmAssignment = async (bookingId: string, providerId: string, providerName: string) => {
    ...
    try {
        ...
        const providerDocSnap = await getFirestoreDoc(providerAppDocRef);
        ...
        const currentBookingSnap = await getFirestoreDoc(bookingDocRef);
  ...
  return (
    <div className="space-y-6">
              ...
              */}<TableBody>{/*
                */}{filteredBookings.map((booking) => (
                  <TableRow key={booking.id}>
                    ...
                    <TableCell>{formatDateForDisplay(booking.scheduledDate)}</TableCell>
                      ...
                      {booking.providerId ? (
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="outline" size="xs" className="text-xs h-7" disabled={isUpdatingStatus === booking.id || isLoadingAppSettings}>
                        ...
                        <Button variant="outline" size="xs" className="text-xs h-7" onClick={() => openAssignModal(booking.id!, booking.bookingId)} disabled={isUpdatingStatus === booking.id || isLoadingAppSettings || isAssignModalOpen}>
                    ...
                    <TableCell>
                       <Select value={booking.status} onValueChange={(newStatus) => handleStatusChange(booking, newStatus as BookingStatus)} disabled={isUpdatingStatus === booking.id || isLoadingAppSettings}>
                          <SelectTrigger className="h-8 text-xs min-w-[120px]">{isUpdatingStatus === booking.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <SelectValue placeholder="Set Status" />}</SelectTrigger>
      ...
      {selectedBooking && (<Dialog open={isDetailsModalOpen} onOpenChange={setIsDetailsModalOpen}><DialogContent className="max-w-3xl w-[90vw] max-h-[90vh] flex flex-col p-0"><DialogHeader className="p-6 pb-4 border-b"><DialogTitle className="text-xl">Booking Details: {selectedBooking.bookingId}</DialogTitle><DialogDescription>Review complete information for this booking.</DialogDescription><DialogClose asChild><Button variant="ghost" size="icon" className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground"><XIcon className="h-4 w-4" /><span className="sr-only">Close</span></Button></DialogClose></DialogHeader><div className="overflow-y-auto flex-grow p-6"><BookingDetailsModalContent booking={selectedBooking} /></div><div className="p-6 border-t flex justify-end"><DialogClose asChild><Button type="button" variant="outline">Close</Button></DialogClose></div></DialogContent></Dialog>)}
