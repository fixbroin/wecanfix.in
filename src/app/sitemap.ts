...
async function getSitemapEntries(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    const blogSnapshot = await getDocs(query(collection(db, 'blogPosts'), where('isPublished', '==', true)));
  ...
  } catch (e) { console.error("Sitemap: Error fetching blog posts:", e); throw e; }
