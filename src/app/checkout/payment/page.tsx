...
export default function PaymentPage() {
  ...
  useEffect(() => {
    ...
    loadInitialData();
  }, [isMounted, toast, isLoadingAppSettings, appConfig, pathname, currentUser, isCancellationFeeMode]);
  ...
  useEffect(() => {
    ...
    if (appConfig.enableMinimumBookingPolicy && typeof appConfig.minimumBookingAmount === 'number' && typeof appConfig.visitingChargeAmount === 'number') {
      ...
      if (appConfig.minimumBookingPolicyDescription) currentPolicyMessage = appConfig.minimumBookingPolicyDescription.replace(/{MINIMUM_BOOKING_AMOUNT}/g, appConfig.minimumBookingAmount.toString()).replace(/{VISITING_CHARGE}/g, (appConfig.visitingChargeAmount || 0).toString());
    }
  ...
  }, [cartEntries, serviceDetailsMap, appConfig, isLoadingCartDetails, isLoadingAppSettings, appliedPromoCode, allFetchedPromoCodes, isCancellationFeeMode]);
  ...
  const handleRemovePromoCode = () => { setAppliedPromoCode(null); setPromoCodeInput(""); localStorage.removeItem('wecanfixAppliedPromoCode'); toast({ title: "Promo Removed", description: "Discount has been removed." }); };
  ...
  const handlePaymentAction = async () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('wecanfixPaymentMethod', paymentMethod);
    ...
    if (paymentMethod === 'later' && !isCancellationFeeMode) { router.push('/checkout/thank-you'); return; }

    if (!onlinePaymentEnabled || !appConfig.razorpayKeyId) { toast({ title: isCancellationFeeMode ? "Online Payment Required" : "Online Payments Disabled", description: "Online payments are currently not available or configured.", variant: "destructive" }); setIsProcessingPayment(false); hideLoading(); return; }
    ...
    try {
      ...
      const paymentDescription = isCancellationFeeMode && cancellationFeeDetails?.humanReadableBookingId
        ? `Cancellation Fee for Booking ${cancellationFeeDetails.humanReadableBookingId}`
        : "Service Booking Payment";

      const options = {
        key: appConfig.razorpayKeyId, amount: Math.round(totalAmountDue * 100), currency: "INR",
        name: appConfig.websiteName || "wecanfix Services", description: paymentDescription,
        handler: (response: any) => {
          localStorage.setItem('razorpayPaymentId', response.razorpay_payment_id);
          if (response.razorpay_order_id) localStorage.setItem('razorpayOrderId', response.razorpay_order_id);
          if (response.razorpay_signature) localStorage.setItem('razorpaySignature', response.razorpay_signature);
      ...
      rzp.open();
    } catch (error) { toast({ title: "Payment Error", description: "An unexpected error occurred.", variant: "destructive" }); setIsProcessingPayment(false); hideLoading(); }
  };
