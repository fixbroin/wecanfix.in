...
import { collection, addDoc, Timestamp, doc, getDoc, runTransaction, query, where, getDocs, limit, updateDoc } from "firebase/firestore";
import type { FirestoreBooking, BookingServiceItem, FirestoreService, FirestorePromoCode, AppSettings, AppliedPlatformFeeItem, FirestoreNotification } from '@/types/firestore';
...
interface DisplayBookingDetails extends Omit<FirestoreBooking, 'services' | 'createdAt' | 'updatedAt' | 'appliedPlatformFees'> {
  id?: string;
...
export default function ThankYouPage() {
  ...
  useEffect(() => {
    ...
    const processPage = async () => {
      ...
      const paymentId = localStorage.getItem('razorpayPaymentId'); 

      if (isProcessingCancellationFee && bookingFirestoreDocIdForCancellation && feeAmountStr && paymentId) {
        ...
        try {
          ...
          if (originalBookingSnap.exists()) {
            ...
            toast({ title: "Booking Cancelled", description: `Cancellation fee of ₹${feeAmount.toFixed(2)} paid. Booking ${originalBookingData.bookingId} has been cancelled.` });
          } else {
            toast({ title: "Error", description: "Original booking not found for cancellation update.", variant: "destructive" });
          }

        } catch (error) {
          console.error("Error updating booking status to Cancelled:", error);
          toast({ title: "Error", description: "Failed to update booking status after fee payment.", variant: "destructive"});
        } finally {
      ...
      if (cartEntriesFromStorage.length === 0 && !lastBookingIdFromStorage) {
        toast({ title: "No Active Booking", description: "No active booking process found. Redirecting to home.", variant: "default" });
        router.push('/'); setIsLoadingPage(false); return;
      }
      if (cartEntriesFromStorage.length === 0 && lastBookingIdFromStorage) {
        toast({ title: "Booking Processed", description: "This booking has already been processed. Redirecting to My Bookings.", variant: "default" });
        router.push('/my-bookings'); setIsLoadingPage(false); return;
      }

      try {
        const newBookingId = generateBookingId();
        let customerEmail = "customer@example.com", paymentMethod = "Awaiting confirmation", scheduledDateStored = new Date().toLocaleDateString('en-CA'), scheduledTimeSlot = "10:00 AM";
        ...
        let storedRazorpayPaymentId: string | undefined, storedRazorpayOrderId: string | undefined, storedRazorpaySignature: string | undefined;
        ...
        const serviceItemsPromises = cartEntriesFromStorage.map(async (entry) => {
          ...
          if (serviceSnap.exists()) {
            ...
            const displayedPricePerUnit = (serviceData.discountedPrice !== undefined && serviceData.discountedPrice < serviceData.price) ? serviceData.discountedPrice : serviceData.price;
            sumOfDisplayedItemPrices += displayedPricePerUnit * entry.quantity;
            const itemTaxRate = (serviceData.taxPercent || 0) > 0 ? (serviceData.taxPercent || 0) : 0;
            const basePricePerUnit = getBasePriceForInvoice(displayedPricePerUnit, serviceData.isTaxInclusive === true, itemTaxRate);
            const taxAmountForItem = (basePricePerUnit * entry.quantity) * (itemTaxRate / 100);
            return { serviceId: entry.serviceId, name: serviceData.name, quantity: entry.quantity, pricePerUnit: displayedPricePerUnit, discountedPricePerUnit: serviceData.discountedPrice, isTaxInclusive: serviceData.isTaxInclusive === true, taxPercentApplied: itemTaxRate, taxAmountForItem: taxAmountForItem };
          } return null;
        });
        const resolvedServiceItems = (await Promise.all(serviceItemsPromises)).filter(item => item !== null) as BookingServiceItem[];
        ...
        let baseSubTotalForBooking = 0; resolvedServiceItems.forEach(item => { const basePrice = getBasePriceForInvoice(item.pricePerUnit, item.isTaxInclusive, item.taxPercentApplied); const itemBaseTotal = basePrice * item.quantity; if (!isNaN(itemBaseTotal)) baseSubTotalForBooking += itemBaseTotal; });
        let displayedVisitingCharge = 0; let baseVisitingChargeForBooking = 0; const subtotalForVcPolicyCheck = sumOfDisplayedItemPrices - (bookingDiscountAmount || 0);
        ...
        const totalAmountForBooking = (baseSubTotalForBooking + baseVisitingChargeForBooking - (bookingDiscountAmount || 0) + totalBasePlatformFees) + totalTaxForBooking;
        ...
        const newBookingData: Omit<FirestoreBooking, 'id'> = {
          ...
          scheduledDate: scheduledDateStored, // Storing YYYY-MM-DD
          scheduledTimeSlot, services: resolvedServiceItems, subTotal: baseSubTotalForBooking,
          ...
          paymentMethod, status: bookingStatus,
          ...(storedRazorpayPaymentId && { razorpayPaymentId: storedRazorpayPaymentId }),
          ...(storedRazorpayOrderId && { razorpayOrderId: storedRazorpayOrderId }),
          ...(storedRazorpaySignature && { razorpaySignature: storedRazorpaySignature }),
        ...
        if (typeof window !== 'undefined') { if (appConfig.googleTagManagerId && window.dataLayer) { window.dataLayer.push({ ecommerce: null }); window.dataLayer.push({ event: 'purchase', ecommerce: { transaction_id: transactionId, value: transactionValue, currency: 'INR', items: resolvedServiceItems.map(item => ({ item_id: item.serviceId, item_name: item.name, price: item.pricePerUnit, quantity: item.quantity, discount: item.discountedPricePerUnit !== undefined ? item.pricePerUnit - item.discountedPricePerUnit : 0 })) } }); } if (appConfig.googleTagId && typeof window.gtag === 'function') { if (appConfig.googleTagId.startsWith('AW-') && appConfig.googleAdsConversionLabel) { window.gtag('event', 'conversion', { 'send_to': `${appConfig.googleTagId}/${appConfig.googleAdsConversionLabel}`, 'value': transactionValue, 'currency': 'INR', 'transaction_id': transactionId }); } else if (appConfig.googleTagId.startsWith('G-') && !appConfig.googleTagManagerId) { window.gtag('event', 'purchase', { transaction_id: transactionId, value: transactionValue, currency: 'INR', items: resolvedServiceItems.map(item => ({ item_id: item.serviceId, item_name: item.name, price: item.pricePerUnit, quantity: item.quantity })) }); } } }
        const emailFlowInput: BookingConfirmationEmailInput = { bookingId: newBookingData.bookingId, customerName: newBookingData.customerName, customerEmail: newBookingData.customerEmail, customerPhone: newBookingData.customerPhone, addressLine1: newBookingData.addressLine1, addressLine2: newBookingData.addressLine2, city: newBookingData.city, state: newBookingData.state, pincode: newBookingData.pincode, scheduledDate: formatDateForDisplay(newBookingData.scheduledDate), scheduledTimeSlot: newBookingData.scheduledTimeSlot, services: newBookingData.services.map(s => ({ serviceId: s.serviceId, name: s.name, quantity: s.quantity, pricePerUnit: s.pricePerUnit, discountedPricePerUnit: s.discountedPricePerUnit })), subTotal: baseSubTotalForBooking, visitingCharge: displayedVisitingCharge, discountAmount: newBookingData.discountAmount, discountCode: newBookingData.discountCode, taxAmount: totalTaxForBooking, totalAmount: totalAmountForBooking, paymentMethod: newBookingData.paymentMethod, status: newBookingData.status, smtpHost: appConfig.smtpHost, smtpPort: appConfig.smtpPort, smtpUser: appConfig.smtpUser, smtpPass: appConfig.smtpPass, senderEmail: appConfig.senderEmail, appliedPlatformFees: newBookingData.appliedPlatformFees?.map(fee => ({ name: fee.name, amount: fee.calculatedFeeAmount + fee.taxAmountOnFee })) };
    ...
    processPage();
  }, [isMounted, toast, currentUser, router, appConfig, isLoadingAppSettings, hideLoading]);
  ...
  return (
    <div className="max-w-2xl mx-auto px-2 sm:px-0">
      ...
      <Card className="shadow-lg text-center">
        ...
        <CardFooter className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center pt-4 sm:pt-6"><Link href="/" passHref><Button size="lg" variant="outline" className="w-full sm:w-auto text-sm sm:text-base"><Home className="mr-2 h-4 w-4" /> Go to Home</Button></Link><Link href="/my-bookings" passHref><Button size="lg" className="w-full sm:w-auto text-sm sm:text-base"><ListOrdered className="mr-2 h-4 w-4" /> Go to My Bookings</Button></Link></CardFooter>
