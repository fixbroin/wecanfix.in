...
export interface FirestoreUser {
  ...
  roles?: ('customer' | 'provider' | 'admin')[]; // For role-based access control
...
export interface GlobalWebSettings {
  ...
  adminUserUidForChat?: string; // New: Specify a UID for the admin chat user.
  isCookieConsentEnabled?: boolean; // New for cookie consent
  cookieConsentMessage?: string; // New for cookie consent banner message
  cookiePolicyContent?: string; // New for cookie policy page content (HTML)
  updatedAt?: Timestamp;
}
...
export interface AppSettings {
  ...
  // Chat settings from GlobalWebSettings might be used here
...
// User Notifications
export type NotificationType = 'success' | 'info' | 'warning' | 'error' | 'booking_update' | 'admin_alert' | 'provider_app_status'; // Added 'provider_app_status'
...
export interface FirestorePopupInquiry {
  ...
  email?: string;
  phone?: string;
  message?: string; // For popups that might have a message field
...
export interface FirestoreInvoice {
  ...
  taxPercent?: number; // e.g., 18 for 18%
  taxAmount?: number; // Calculated: (Subtotal - DiscountAmount) * (TaxPercent / 100)
...
export interface PinCodeAreaMapping {
  ...
  areaName: string; // Common name for this area
  order: number; // For sorting in admin display
  createdAt: Timestamp;
  updatedAt?: Timestamp;
}
...
export interface ProviderApplication {
  ...
  workPinCodes?: string[]; // Array of selected PIN code strings
