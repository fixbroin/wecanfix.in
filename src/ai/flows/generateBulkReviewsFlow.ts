...
const GeneratedReviewSchema = z.object({
  userName: z.string().describe("A realistic, common Indian name (e.g., Priya Sharma, Rohan Kumar)."),
  rating: z.number().min(3).max(5).describe("A rating between 3 and 5."),
  comment: z.string().describe("A realistic, concise review comment (10-40 words). Comments should be a mix of very positive, moderately positive, and neutral tones. They should sound natural and authentic."),
});
...
2.  **rating**: An integer rating between 3 and 5. The distribution should be mostly 4s and 5s, with a few 3s.
3.  **comment**: A short, natural-sounding review comment between 10 and 40 words. The tone should vary (e.g., "Good work.", "Very professional and quick service!", "Satisfied with the job, but was a bit late.").
