...
import ServiceCard from '@/components/service/ServiceCard'; // Corrected import path
...
export default function CategoryPageClient({ categorySlug, citySlug, areaSlug, breadcrumbItems: initialBreadcrumbItems }: CategoryPageClientProps) {
  ...
  const [category, setCategory] = useState<FirestoreCategory | null>(() => getCache(cacheKey)?.category || null);
  const [subCategoriesWithServices, setSubCategoriesWithServices] = useState<EnrichedSubCategory[]>(() => getCache(cacheKey)?.subCategories || []);
  ...
  const [seoPageH1, setSeoPageH1] = useState<string | null>(() => getCache(cacheKey)?.h1 || null); 
  const [displayPageH1, setDisplayPageH1] = useState<string | null>(() => getCache(cacheKey)?.displayH1 || null); 
  ...
  useEffect(() => {
    const fetchCategoryDataAndServices = async () => {
      ...
      if (cachedData) {
        ...
        if (cachedData.subCategories.length > 0) setActiveSubCategorySlug(cachedData.subCategories[0].slug);
        if (initialBreadcrumbItems) setBreadcrumbItems(initialBreadcrumbItems);
        setIsLoading(false);
        return;
      }
      ...
      {subCategoriesWithServices.length > 0 ? (
        <>
          ...
          <div className="space-y-12">
            {subCategoriesWithServices.map((subCat, subCatIndex) => {
              return (
                <section 
                  key={subCat.id} 
                  ...
                  {subCat.services.length > 0 ? (
                    <div className="grid grid-cols-1 gap-4">
                      {subCat.services.map((service, serviceIndex) => (
                        <ServiceCard key={service.id} service={service} priority={subCatIndex === 0 && serviceIndex < 4} />
                      ))}
                    </div>
                  ) : (
        ...
        <Popover open={isSubCategoryPopoverOpen} onOpenChange={setIsSubCategoryPopoverOpen}>
          <PopoverTrigger asChild>
            <Button
              variant="default"
              size="icon"
              className="fixed bottom-[9.5rem] right-2 h-14 w-14 rounded-full shadow-xl z-40 flex items-center justify-center"
