...
import { Star, Clock, ListChecks, Loader2 } from 'lucide-react';
...
export default function HomePageClient({ citySlug, areaSlug, breadcrumbItems }: HomePageClientProps) {
  ...
  const [isLoadingFeaturesConfig, setIsLoadingFeaturesConfig] = useState(true);

  const [popularServices, setPopularServices] = useState<FirestoreService[]>([]);
  const [isLoadingPopular, setIsLoadingPopular] = useState(false);
  const [recentServices, setRecentServices] = useState<FirestoreService[]>([]);
  const [isLoadingRecent, setIsLoadingRecent] = useState(false);
  const [categoryWiseServicesData, setCategoryWiseServicesData] = useState<Array<{category: FirestoreCategory, services: FirestoreService[] }>>([]);
  const [isLoadingCategoryWise, setIsLoadingCategoryWise] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchFeaturesConfigAndData = async () => {
        setIsLoadingFeaturesConfig(true);
        let currentFeaturesConfig = defaultFeaturesConfig;
        try {
        ...
        if (currentFeaturesConfig.showRecentlyAddedServices) {
            setIsLoadingRecent(true);
            try {
                const servicesQuery = query(collection(db, "adminServices"), where("isActive", "==", true), orderBy("createdAt", "desc"), limit(100));
                const snapshot = await getDocs(servicesQuery);
                setRecentServices(snapshot.docs.map(d => ({...d.data(), id: d.id } as FirestoreService)));
            } catch (e) { console.error("Error fetching recent services:", e); }
            setIsLoadingRecent(false);
        }

        if (currentFeaturesConfig.showMostPopularServices) {
            setIsLoadingPopular(true);
            try {
                // Placeholder: Using rating and reviewCount for "popularity"
                const servicesQuery = query(collection(db, "adminServices"), where("isActive", "==", true), orderBy("rating", "desc"), orderBy("reviewCount", "desc"), limit(100));
                const snapshot = await getDocs(servicesQuery);
                setPopularServices(snapshot.docs.map(d => ({...d.data(), id: d.id } as FirestoreService)));
            } catch (e) { console.error("Error fetching popular (placeholder) services:", e); }
            setIsLoadingPopular(false);
        }

        if (currentFeaturesConfig.showCategoryWiseServices) {
            setIsLoadingCategoryWise(true);
            try {
                ...
                if (enabledCategoryIds.length > 0) {
                    ...
                    const categoryServicesPromises = enabledCategories.map(async (cat) => {
                        // Fetch subCategory IDs for the current parent category
  ...
  if (!isMounted || isLoadingAppSettings || isLoadingPageData || isLoadingFeaturesConfig) {
    return (
        <div className="flex flex-col">
            {/* Simplified Skeleton for Breadcrumbs and Header */}
            <div className="container mx-auto px-4 pt-4 md:pt-6 mb-4 md:mb-6">
                <Skeleton className="h-5 w-1/3" />
            </div>
            {/* Skeleton for Hero Carousel or its space */}
            ...
            <section className="py-8 md:py-12 bg-secondary/30">
                <div className="container mx-auto px-4">
                    ...
                    <div className="text-center mt-8 md:mt-12"><Skeleton className="h-12 w-36 mx-auto" /></div>
                </div>
            </section>
             {/* Skeleton for other sections - just one representative example */}
            <section className="py-8 md:py-12">
                <div className="container mx-auto px-4">
                    <div className="text-center mb-8 md:mb-12"><Skeleton className="h-8 w-1/2 mx-auto" /></div>
                    <div className="flex space-x-4 overflow-hidden p-1 pb-3">
                        {[...Array(4)].map((_, i) => (<Card key={i} className="w-[200px] sm:w-[220px] flex-shrink-0"><Skeleton className="h-28 sm:h-32 w-full" /><CardContent className="p-2 sm:p-3"><Skeleton className="h-4 w-3/4 mb-1" /><Skeleton className="h-3 w-1/2" /></CardContent></Card>))}
                    </div>
                </div>
            </section>
        </div>
