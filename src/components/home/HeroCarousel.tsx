...
export function HeroCarousel() {
  ...
  const emblaPlugins = React.useMemo(() => {
    if (autoplayEnabled && slides.length > 1) {
      return [Autoplay({ delay: autoplayDelay, stopOnInteraction: true, stopOnMouseEnter: true })];
    }
    return [];
  ...
  const handleSlideNavigation = useCallback((type?: SlideButtonLinkType | null, value?: string | null) => {
    ...
    if (intendedHref.startsWith('http')) { // External link
      ...
      // No showLoading() needed here for the original tab.
  ...
  }, [user, triggerAuthRedirect, router, showLoading, getButtonLink]);
  ...
  return (
    <div className="px-0 sm:px-0">
      ...
      >
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/40 to-transparent rounded-lg flex flex-col items-center justify-end text-center p-4 pb-6 sm:p-6 sm:pb-8 md:p-8 md:pb-10 lg:p-10 lg:pb-12">
                  <div className="max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl">
                    ...
                    {slide.buttonText && slide.buttonText.trim() !== "" && slide.buttonLinkType && slide.buttonLinkValue && (
                      <Button 
                        size="default" 
                        className="text-xs sm:text-sm h-9 md:h-10 bg-primary hover:bg-primary/90 text-primary-foreground px-4 md:px-6"
                        onClick={(e) => { e.stopPropagation(); handleSlideNavigation(slide.buttonLinkType, slide.buttonLinkValue);}}
                      >
                        {slide.buttonText}
                      </Button>
                    )}
        ...
        {slides.length > 1 && (
          <>
            <CarouselPrevious className="absolute left-1 sm:left-2 top-1/2 -translate-y-1/2 h-6 w-6 sm:h-8 sm:w-8" />
            <CarouselNext className="absolute right-1 sm:right-2 top-1/2 -translate-y-1/2 h-6 w-6 sm:h-8 sm:w-8" />
          </>
        )}
      </Carousel>
    </div>
