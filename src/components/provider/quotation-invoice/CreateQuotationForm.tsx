...
import { collection, getDocs, addDoc, Timestamp, query, orderBy, doc, setDoc, updateDoc, getDoc, where } from "firebase/firestore";
...
export default function CreateQuotationForm({ initialData, onSaveSuccess }: CreateQuotationFormProps) {
  ...
  useEffect(() => {
    ...
    const fetchProviderCustomers = async () => {
      ...
      try {
        ...
        const customerIds = [...new Set(bookingsSnapshot.docs.map(doc => doc.data().userId))];
        
        if (customerIds.length > 0) {
            const usersQuery = query(collection(db, "users"), where("uid", "in", customerIds));
            const usersSnapshot = await getDocs(usersQuery);
            const customerUsers = usersSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id } as FirestoreUser));
  ...
  const handleAction = async (actionType: 'send' | 'download') => {
    ...
    finally { if (actionType === 'send') setIsSending(null); else setIsDownloading(false); }
  };
                ...
                <FormLabel>Select Your Customer (Optional)</FormLabel>
                 <Popover open={isUserSearchOpen} onOpenChange={setIsUserSearchOpen}>
                  <PopoverTrigger asChild>
                    <Button variant="outline" role="combobox" className="w-full justify-between h-10" disabled={isSaving || isLoadingUsers}>
                        {selectedUserForDisplay ? (<span className="truncate">{selectedUserForDisplay.name}</span>) : "Select a customer..."}
                  ...
                  <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-0">
                     ...
                     <ScrollArea className="h-48">
                        {isLoadingUsers ? <div className="p-2 text-center text-sm">Loading...</div> :
                         ...
                         ) : <div className="p-2 text-center text-sm">No customers found for you.</div>
                        }
                     </ScrollArea>
                  </PopoverContent>
                </Popover>
