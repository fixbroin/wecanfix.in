...
export default function CreateInvoiceForm({ initialData, onSaveSuccess }: CreateInvoiceFormProps) {
  ...
  useEffect(() => {
    const sub = form.watch((value, { name }) => {
        ...
        const currentDiscountPercent = value.discountPercent || 0;
        ...
        const currentTaxPercent = value.taxPercent || 0;
        ...
        const currentAmountPaid = value.amountPaid || 0;
  ...
  const onSubmit = async (data: CreateInvoiceFormData) => {
    ...
    try {
      ...
      if (isEditing && initialData?.id) {
        ...
        savedItem = { ...initialData, ...invoiceDataForFirestore } as FirestoreInvoice;
