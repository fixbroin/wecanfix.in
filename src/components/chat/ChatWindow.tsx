  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isGlobalAiEnabled) {
        ...
        const historyForAi: ChatHistoryItem[] = messages.map(msg => ({
          role: msg.senderType === 'user' ? 'user' : 'model', // Map all non-user to 'model'
          content: [{ text: msg.text || '' }]
        }));

        const aiResponse = await chatWithAgent({ history: historyForAi, message: tempNewMessage });
        setIsAiTyping(false);
