...
export default function ServiceDetailPageClient({
  ...
  const [service, setService] = useState<ClientServiceData | null>(initialServiceData || getCache(cacheKey)?.service || null);
  const [h1Title, setH1Title] = useState<string | null>(initialH1Title || (initialServiceData ? (initialServiceData.h1_title || initialServiceData.name) : (getCache(cacheKey)?.h1Title || null)));
  const [quantity, setQuantity] = useState(0);
  const [isLoading, setIsLoading] = useState(!service);
  const [serviceReviews, setServiceReviews] = useState<FirestoreReview[]>(() => getCache(cacheKey)?.reviews || []);
  const [isLoadingReviews, setIsLoadingReviews] = useState(() => !getCache(cacheKey)?.reviews);
  ...
  useEffect(() => {
    ...
    if(cachedData) {
        ...
        if (cachedData.reviews) {
          fetchReviewsForService(cachedData.service.id);
        }
        return;
    }
              ...
              <div>
                {service.discountedPrice && service.discountedPrice < service.price ? (
                  <div className="flex items-baseline gap-1 sm:gap-2">
                    <p className="text-2xl sm:text-3xl font-bold text-foreground">₹{service.discountedPrice.toLocaleString()}</p>
