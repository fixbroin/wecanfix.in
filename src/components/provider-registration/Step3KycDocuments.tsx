...
const step3KycSchema = z.object({
  aadhaarNumber: z.string()
    .regex(/^[2-9]{1}[0-9]{3}[0-9]{4}[0-9]{4}$/, "Invalid Aadhaar number (must be 12 digits, not starting with 0 or 1)."),
  panNumber: z.string()
...
export default function Step3KycDocuments({
  ...
  const handleSubmit = async (data: Step3FormData) => {
    if (!aadhaarFront.file && !aadhaarFront.existingUrl) {
      toast({ title: "Aadhaar Front Image Required", description: "Please upload the front image of your Aadhaar card.", variant: "destructive" });
      return;
    }
    if (!aadhaarBack.file && !aadhaarBack.existingUrl) {
      toast({ title: "Aadhaar Back Image Required", description: "Please upload the back image of your Aadhaar card.", variant: "destructive" });
      return;
    }
    if (!panFront.file && !panFront.existingUrl) {
      toast({ title: "PAN Card Image Required", description: "Please upload the front image of your PAN card.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      // AADHAAR
      const [frontAadhaar, backAadhaar] = await Promise.all([
      ...
      applicationStepData.aadhaar = {
        ...
        frontImageUrl: frontAadhaar.url!, frontImageFileName: frontAadhaar.fileName || undefined,
        backImageUrl: backAadhaar.url!, backImageFileName: backAadhaar.fileName || undefined,
        verified: initialData.aadhaar?.verified || false,
      };

      // PAN
      const frontPan = await uploadFile(panFront, 'pan', 'front');
      applicationStepData.pan = {
        docType: 'pan', docNumber: data.panNumber,
        frontImageUrl: frontPan.url!, frontImageFileName: frontPan.fileName || undefined,
  ...
  const renderFileInput = (
    ...
    return (
    <FormItem>
      <FormLabel htmlFor={inputId}>{label} <span className="text-destructive">*</span></FormLabel>
      <div className={`relative w-full ${aspectRatio || 'aspect-video'} rounded-md border border-dashed bg-muted/30 flex items-center justify-center`}>
        {isValidImageSrc(fileState.previewUrl) ? (
        ...
        <CardContent className="space-y-6">
          <Card className="p-4">
            ...
            <CardContent className="p-0 space-y-4">
              <FormField control={form.control} name="aadhaarNumber" render={({ field }) => (<FormItem><FormLabel>Aadhaar Number <span className="text-destructive">*</span></FormLabel><FormControl><Input placeholder="Enter 12-digit Aadhaar" {...field} disabled={effectiveIsSaving} /></FormControl><FormMessage /></FormItem>)} />
          ...
          <Card className="p-4">
            ...
            <CardContent className="p-0 space-y-4">
              <FormField control={form.control} name="panNumber" render={({ field }) => (<FormItem><FormLabel>PAN Number <span className="text-destructive">*</span></FormLabel><FormControl><Input placeholder="Enter 10-digit PAN" {...field} disabled={effectiveIsSaving} /></FormControl><FormMessage /></FormItem>)} />
