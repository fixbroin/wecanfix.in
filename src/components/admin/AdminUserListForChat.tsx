...
export default function AdminUserListForChat({
  onSelectUser,
  selectedUserId,
  scrollAreaHeightClass = "h-[calc(100vh-25rem)] sm:h-[calc(100vh-22rem)]"
  ...
  useEffect(() => {
    ...
    const sessionIdsForQuery = userIds.map(uid => getChatSessionId(uid, adminUser.uid!)); // adminUser.uid is now checked
  ...
  if (users.length === 0) {
    return (
        <div className="border rounded-lg p-4 h-full flex flex-col items-center justify-center text-center">
            ...
            <p className="text-muted-foreground">No registered users found to chat with.</p>
        </div>
    );
  }

  return (
    <Card className="h-full flex flex-col shadow-none border-0 md:border md:shadow-md rounded-none md:rounded-lg">
