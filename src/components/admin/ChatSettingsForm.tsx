...
import { Loader2, Save, Volume2, Trash2, UploadCloud } from "lucide-react";
...
const isFirebaseStorageUrl = (url: string | null | undefined): boolean => !!url && typeof url === 'string' && url.includes("firebasestorage.googleapis.com/v0/b/wecanfix-2.firebasestorage.app/o/chat_sounds");
...
export default function ChatSettingsForm() {
  ...
  const playCurrentSound = () => {
    if (audioPlayerRef.current) {
        ...
        if (soundToPlay) {
            // If the src is already set to the target and it's a new file preview (blob),
            // it might need a little help to replay if user clicks preview multiple times for the same new file.
            // For existing URLs or default, just setting src and playing should be fine.
            if (audioPlayerRef.current.src === soundToPlay && soundToPlay.startsWith('blob:')) {
                audioPlayerRef.current.currentTime = 0; // Rewind if it's the same blob
            } else {
